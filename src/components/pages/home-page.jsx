  ...
  return (
    <main className="flex flex-col gap-10 sm:gap-20 py-10 sm:py-20">
      ...
      <Carousel
        ...
        <CarouselContent className="flex gap-5 sm:ga-20 items-center">
          {companies.map(({ name, path, id }) => (
            <CarouselItem key={id} className="basis-1/3 lg:basis-1/6">
              <img src={path} alt={name} className="h-12 object-contain mx-auto" />
            </CarouselItem>
          ))}
        </CarouselContent>
      </Carousel>
      ...
      <Accordion type="single" collapsible>
        {
          faqs.map((faq, index) => {
            return (
              <AccordionItem index={index} value={`item-${index-1}`}>
